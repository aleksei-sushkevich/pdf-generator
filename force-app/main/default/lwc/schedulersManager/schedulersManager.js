...
export default class SchedulersManager extends LightningElement {
    ...
    getAllBatches(){
        // getSchedulersData()
        getSchedulersData({batchName : this.batchName})
        .then(data => { 
    ...
    abortBatch(){
        ...
        abortBatch({cronId : this.cronId})
        .then( () =>{     
            this.dispatchEvent(
                new ShowToastEvent({
                    // message: 'Scheduler ' + this.batches[this.index].label + ' was deleted',
                    message: 'Scheduler ' + this.batchName + ' was deleted',
    ...
    scheduleBatch(){
        ...
        scheduleBatch({cronExpr : this.scheduleValue, className : this.batchName})
        .then( () =>{     
            this.dispatchEvent(
                new ShowToastEvent({
                    // message: this.batches[this.index].label + ' was scheduled at ' + this.scheduleValue,
                    message: this.batchName + ' was scheduled at ' + this.scheduleValue,
                    variant: SUCCESS_VARIANT
                })
            );
            this.getAllBatches(); 
    ...
    runOnce(){
        ...
        runOnce({className : this.batchName})
        .then( () =>{      
            this.dispatchEvent(
                new ShowToastEvent({
                    // message:this.batches[this.index].label + ' was launched',
                    message: this.batchName + ' was launched',
                    variant: SUCCESS_VARIANT
                })
            );
            this.getAllBatches(); 
